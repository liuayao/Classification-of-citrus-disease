...
from res2net import Res2NetBlock as res2net_block
from SENET import SEblock as se_block
...
class Block(nn.Module):
    def __init__(self, in_channel, k, use_bottleneck=True):
        ...
        # self.res_block= res2net_block(k,k)
        # self.se_block= se_block(k)

    def forward(self, x):
        # x=torch.cat((x, self.block(x)), 1)
        # x=self.res_block(x)
...
class DenseNet(nn.Module):
    ...
    def __init__(self, in_channel, k, num_blocks, theta, n_classes, block, use_bottleneck):
        ...
        for i, num_block in enumerate(num_blocks):
            ...
            self.features.add_module(f'resblock_layer_{i}', res2net_block(inner_channel, inner_channel))
            self.features.add_module(f'seblock_layer_{i}', se_block(inner_channel))
    ...
    def forward(self, x):
        ...
        return nn.Softmax(dim=-1)(self.linear(out))

def densenet121(use_bottleneck=True):
    return DenseNet(3, 8, [2, 4, 8, 4], 0.5, 5, Block, use_bottleneck)
